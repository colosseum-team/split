...
(async () => {
  ...
  if (created.contractHash !== created2.contractHash) {
    console.warn("  ! hashes differ — likely deadline timestamps differ. Acceptable: hash includes deadline.");
  } else {
  ...
  const brief = await http("POST", `/contracts/${created.id}/dispute-output`, {
    ...
    body: {
      ...
      modelVersion: "demo-1",
      result: {
        case_summary: "Customer claims monochrome was missing. User submitted 3 concepts in B/W and color.",
  ...
  console.log("status:", resolved.status, "outcome:", resolved.disputeOutcome, "onchain:", resolved.onchain);

  console.log("=== list AI outputs ===");
  const outputs = await http(
    "GET",
    `/contracts/${created.id}/ai-outputs`,
    { token: customerToken },
  );
  console.log("outputs count:", outputs.length, "kinds:", outputs.map((o) => o.kind));
