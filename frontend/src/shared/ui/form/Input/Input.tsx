...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    if (hasValidationError) {
      inputStateClasses =
        'bg-(--color-surface-raised) md:bg-(--color-surface-muted) border border-(--color-state-danger) text-(--color-text-primary) font-medium md:text-[16px] text-[14px]'
    } else if (isActive) {
      inputStateClasses =
        'bg-(--color-surface-raised) md:bg-(--color-surface-muted) border text-(--color-text-primary) font-medium md:text-[16px] text-[14px]'
      borderStyle.borderColor = activeBorderColor || 'var(--color-brand-accent-border)'
    } else if (isFilled) {
      inputStateClasses =
        'bg-(--color-surface-raised) md:bg-(--color-surface-muted) border border-(--color-border-subtle) text-(--color-text-primary) font-medium md:text-[16px] text-[14px]'
    } else {
      inputStateClasses =
        'bg-(--color-surface-muted) border border-(--color-border-subtle) placeholder:text-(--color-text-muted) font-medium md:text-[16px] text-[14px]'
    }

    const inputClasses = `w-full h-[53px] rounded-[var(--radius-lg)] px-5 py-4 focus:outline-none transition-colors ${inputStateClasses} ${className}`
      ...
      <div className="w-full relative">
        {label && (
          <label htmlFor={inputId} className={labelClasses}>
            {label}
            {props.required && <span className="text-(--color-state-danger)">*</span>}
          </label>
        )}

        {shouldShowRequiredError && requiredErrorPosition === 'top-right' && (
          <div className="absolute top-0 right-0 md:text-[14px] text-[12px] text-(--color-state-danger)">
            Required
          </div>
        )}
        ...
        <div className="relative">
          ...
          {confirmEdit && !isEditing && (
            <button
              ...
              aria-label="Edit"
            >
              <PencilIcon className="w-[18px] h-[18px] text-(--color-text-muted)" />
            </button>
          )}
        </div>
